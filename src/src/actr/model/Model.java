...
public class Model
{
	...
	private int participantNum = getLastParticipant() + 1;
...
//	public static String outputPath = userDirectory+File.separator+"data"+File.separator;
	...
	/**
	 * Compiles a model from a string, while also overriding the given task. The enclosing frame is
	 * also needed to provide a way to print output to the screen.
	...
	/**
	 * Compiles a model from a file. The enclosing frame is also needed to provide a way to print
	 * output to the screen.
	...
	public static Model compile (File file, Frame frame)
	{
		try
		{
			...
			int c;
			while ((c = inputStream.read()) != -1) sw.write (c);
			inputStream.close();
	 ...
	 */
	public Declarative getDeclarative () { return declarative; }
	 ...
	 */
	public Imaginal getImaginal () { return imaginal; }
	 ...
	 */
	public Bold getBold () { return bold; }

	/**
	 * Gets the current buffers.
	 * @return the buffers
	 */
	public Buffers getBuffers () { return buffers; }
	...
	int getCurrentThreadID () { return currentThreadID; }
	int getNewThreadID () { return (++currentThreadID); }

	public Iterator<ParseError> getErrors () { return errors.iterator(); }

	public boolean hasFatalErrors ()
	{
		Iterator<ParseError> it = getErrors();
		while (it.hasNext())
			if (it.next().isFatal()) return true;
		return false;
	}
	 ...
	 * @return <tt>true</tt> if there is an event scheduled for the module, or <tt>false</tt> otherwise
	 */
	public boolean hasEvent (String module) { return events.scheduled (module); }

	/**
	 * Checks whether there is currently an event scheduled for a particular module with a
	 * description that starts with the given prefix.
	 * @param module the module
	 * @param prefix the prefix string
	 * @return <tt>true</tt> if there is an event scheduled for the module and prefix, or <tt>false</tt> otherwise
	 */
	public boolean hasEvent (String module, String prefix) { return events.scheduled (module, prefix); }

	/**
	 * Changes the event time for the given module with a
	 * description that starts with the given prefix.
	 * @param module the module
	 * @param prefix the prefix string
	 ...
	 * @param module the module
	 */
	public void removeEvents (String module) { events.removeModuleEvents (module); }

	/**
	 * Removes all events for the given module with a
	 * description that starts with the given prefix.
	 * @param module the module
	 * @param prefix the prefix string
	 */
	public void removeEvents (String module, String prefix) { events.removeModuleEvents (module, prefix); }

	/**
	 * Runs a model simulation until there are no more events on the event queue,
	 * or until <tt>stop()</tt> is explicitly called.
	 */
	 ...
	 * @param reset <tt>true</tt> to start the model from time zero, or <tt>false</tt> to resume from the current state
	 */
	public void run (boolean reset)
	{
		...
		if (reset)
		{
			...
			addEvent (new Event (0.0, "procedural", "start") {
				public void action() {
					...
					if (bold.brainImaging) bold.start();
				}});
		}
		...
		while (!stop && (events.hasMoreEvents() || runUntilStop))
		{
			...
			if (verboseTrace && !event.getModule().equals("task")
					&& !event.getModule().equals("bold") && !event.getModule().equals(""))
		...
		if (verboseTrace)
		{
			if (events.hasMoreEvents()) output ("------", "stop");
			else output ("------", "done");
		}
		if (frame.traceOut) {
	...
	void incrementalSleep (long ms)
	{
		...
		while (!stop && (ms > 0))
		{
			try { Thread.sleep (Math.min (ms, increment)); }
	 ...
	 */
	public void resume() { run (false); }

	/**
	 * Checks whether the model simulation has completed (i.e., there are no more events in the event queue).
	 * @return <tt>true</tt> if the model has completed, or <tt>false</tt> otherwise
	 */
	public boolean isDone() { return !events.hasMoreEvents(); }

	/**
	 * Checks whether the model verbose trace is on.
	 * @return <tt>true</tt> if verbose trace is on, or <tt>false</tt> otherwise
	 */
	public boolean isVerbose() { return verboseTrace; }

	void setParameter (String parameter, String value, Tokenizer t)
	{
		if (parameter.equals(":esc"))
		{ if (value.equals("nil")) recordWarning ("unsupported parameter value: "+parameter+" "+value, t); }
		...
		else if (parameter.equals(":nu")) procedural.productionCompilationNewUtility = Double.valueOf(value);
		...
		else if (parameter.equals(":tct")) procedural.threadedCognitionTrace = (!value.equals("nil"));

		else if (parameter.equals(":pc-threaded")) procedural.productionCompilationThreaded = (!value.equals("nil"));
		else if (parameter.equals(":pc-add-utilities")) procedural.productionCompilationAddUtilities = (!value.equals("nil"));

		else if (parameter.equals(":emma")) vision.useEMMA = (!value.equals("nil"));
		else if (parameter.equals(":emma-enc-fac")) vision.emmaEncodingTimeFactor = Double.valueOf(value);
		else if (parameter.equals(":emma-enc-exp")) vision.emmaEncodingExponentFactor = Double.valueOf(value);

		else if (parameter.equals(":brain-imaging")) bold.brainImaging = (!value.equals("nil"));
	...
	/**
	 * Sets a global ACT-R parameter (equivalent to <tt>(sgp \<parameter\> \<value\>)</tt> in the model code).
	 * @param parameter the parameter to set, specified as a String and beginning with ':'
	...
	/**
	 * Compiles and runs an ACT-R command specified as a string (e.g., <tt>(add-dm (goal isa task))</tt>).
	...
	{
		if (frame != null) frame.clearOutput();
	}

	void outputError (String s) { output ("Error: " + s); }
	void outputWarning (String s) { output ("Warning: " + s); }
	...
	public void print(List<String> output, String filename) {
		if (filename.contains("behavior"))
			try {
				FileWriter writer = new FileWriter(
						outputPath + filename + String.format("%03d", participantNum) + ".csv");
			...
			try {
				FileWriter writer = new FileWriter(
						outputPath + filename + String.format("%03d", participantNum) + ".txt");
				for (String str : output) {
					writer.write(str);
				}
				writer.close();
