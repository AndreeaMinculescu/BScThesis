...
public class Core
{
	...
	void startup ()
	{
		...
		{
			if (fileToOpen != null) openFrame (new File (fileToOpen));
			else newFrame();
		}
	}

	Preferences getPreferences() { return prefs; }
	PrefDialog getPrefDialog() { return prefDialog; }
	...
	public Frame getFrame (File file)
	{
		for (int i=0 ; i<frames.size() ; i++)
			if (file.equals (frames.elementAt(i).getFile())) return frames.elementAt(i);
		return null;
	}
	...
	{
		for (int i=0 ; i<frames.size() ; i++)
			if (frames.elementAt(i).isActive()) return frames.elementAt(i);
		return null;
	}
	 ...
	 * @return a new frame
	 */
	public Frame newFrame () { return newFrame (null); }
	...
	public Frame newFrame (File file)
	{
		...
		frames.add (frame);
		if (invisibleFrame!=null && invisibleFrame.isVisible()) invisibleFrame.setVisible (false);
		return frame;
	}

	/**
	 * Presents an Open File dialog box to the user and, if a file is selected, opens the file.
	 */
	...
	/**
	 * Opens the given file. If the current frame has an unchanged blank document, the file is loaded
	 * into the current frame; otherwise, the file is loaded into a new frame.
	 * @param file the file
	 */
	public void openFrame (File file)
	{
		...
		{
			frame = currentFrame();
			if (frame!=null && frame.getFile()==null && !frame.getDocument().isChanged()) frame.open (file);
			else newFrame (file);
		}
	}
	...
	public void closeFrame (Frame frame)
	{
		...
		if (frames.size() == 0)
		{
			if (Main.onMac())
			{
				if (invisibleFrame == null) invisibleFrame = new InvisibleFrame (this);
	...
	{
		if (runLock == null) { runLock = frame; return true; }
		else return false;
	}
	synchronized void releaseLock (Frame frame)
	{
		if (runLock == frame) runLock = null;
	}
	...
	/**
	 * Closes all frames, except when a frame is unsaved and the user cancels the save operation.
	 * @return <tt>true</tt> if all frames were closed, or <tt>false</tt> otherwise
	 */
	public boolean closeAllFrames ()
	{
		for (int i=0 ; i<frames.size() ; i++)
			if (! frames.elementAt(i).close()) return false;
		return true;
	}

	/**
	 * Closes all frames and exits the application, except when a frame is unsaved and the user
	 * cancels the save operation.
	 * @return <tt>true</tt> if the quit is successful, or <tt>false</tt> otherwise
	 */
	public boolean quit ()
	{
		if (closeAllFrames()) System.exit (0);
		return false;
	}
