...
/**
 * The class that defines a graphical frame (window) for editing and running an ACT-R model.
 * The frame contains a text editor for the model text, a task panel for displaying
 * the task interface, and an output panel for displaying the text output of the simulation.
 * 
...
public class Frame extends JFrame
{
	...
	Frame (final Core core, File file)
	{
		...
		outputArea.addFocusListener (new FocusListener() {
			public void focusGained(FocusEvent e) { update(); }
			public void focusLost(FocusEvent e) { update(); }
		});
		...
		if (Main.inApplication()) setDefaultCloseOperation (WindowConstants.DO_NOTHING_ON_CLOSE);
		
		addWindowListener (new WindowAdapter() {
			public void windowClosing (WindowEvent e) { core.closeFrame (frame); }
			public void windowActivated(WindowEvent e) { update(); }
			public void windowDeactivated(WindowEvent e) { update(); }
		});
	 ...
	 * @return the file, or <tt>null</tt> if there is none (e.g., the editor has not been saved)
	 */
	public File getFile() { return file; }

	boolean isModelFile() { return file!=null && file.getName().endsWith(".actr"); }
	boolean isBatchFile() { return file!=null && file.getName().endsWith(".batch"); }

	Editor getEditor() { return editor; }
	Document getDocument() { return (editor != null) ? editor.getModelDocument() : null; }
	Model getModel() { return model; }
	Task getTask() { return (model != null) ? model.getTask() : null; }
	 ...
	 * @param file the file
	 * @param suppressStyling true to disable coloring and indenting (for batch processing)
	 */
	...
	/**
	 * Runs the model currently loaded into the editor, possibly resetting it before running.
	 * @param reset flag indicating whether or not to recompile the model before running
	 */
	public void run (final boolean reset)
	{
		if (isBatchFile()) { runBatch(true); return; }
		...
		(new SwingWorker<Object,Object>() {
			public Object doInBackground() {
				...
				if (reset)
				{
					...
					if (model.hasFatalErrors()) model = null;
				}
				else output ("\n> (resume)\n");
				if (model != null)
				{
					...
					if (!speedup.equals("")) model.setParameter (":real-time", speedup);
					model.run (reset);
					hideTask();
				}
	...
	/**
	 * Runs an analysis of the current model. The analysis runs the model the number of times
	 * defined by the task's <tt>analysisIterations()</tt> method and then runs the task's
	 * <tt>analyze()</tt> method on all the iterations of the task, printing the analysis
	 * output to the frame's output panel.
	 */
				...
				if (filenames != null)
				{
					for (int k=0 ; !stop && k<filenames.length ; k++)
					{
						...
						for (int ti=0 ; ti<taskOverrides.length ; ti++)
						{
							...
							if (model == null)
							{ output("Error: Model "+basePath+modelName+" does not exist"); break; }
	...
	/**
	 * Resumes the previous model simulation if possible (equivalent to <tt>run(false)</tt>).
	 */
	public void resume ()
	{
		...
		try
		{
			if (forceSaveAs || file == null)
			{
				...
				while (newFile == null)
				{
					...
					if (file != null) fileDialog.setDirectory (file.getPath());
					else fileDialog.setDirectory (core.getPreferences().getMostRecentPath());
			...
			int c;
			while ((c = sr.read()) != -1) outputStream.write (c);
			outputStream.close();
	...
	void findNext ()
	{
		if (editorFind.isVisible() || editor.hasFocus()) editorFind.findNext();
		else outputFind.findNext();
	}

	void findPrevious ()
	{
		if (editorFind.isVisible() || editor.hasFocus()) editorFind.findPrevious();
		else outputFind.findPrevious();
	}

	void findHide ()
	{
		if (editorFind.isVisible()) editor.grabFocus();
		if (outputFind.isVisible()) outputArea.grabFocus();
		editorFind.setVisible (false);
		navigator.setVisible (true);
		outputFind.setVisible (false);
	}
	...
	{
		if (editorFind.isVisible() || editor.hasFocus()) return editorFind.isFindNextPossible();
		else if (outputFind.isVisible() || outputArea.hasFocus()) return outputFind.isFindNextPossible();
		else return false;
	}
	...
	boolean closing ()
	{
		if (model!=null && core.hasLock(frame)) stop();
		if (Main.inApplication() && getDocument().isChanged())
		{
			SaveDialog saveDialog = new SaveDialog (frame, getFileName());
			if (saveDialog.cancel) return false;
			else if (saveDialog.save) save (false);
		}
		return true;
	}
	...
	{
		output ("\n> (dm)\n");
		if (model!=null) model.outputDeclarative();
	}
	...
	{
		output ("\n> (p)\n");
		if (model!=null) model.outputProcedural();
	}
